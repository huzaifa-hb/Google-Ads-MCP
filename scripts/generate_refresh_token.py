"""Generate Google Ads OAuth credentials for a single-owner deployment.

By default, the refresh token is printed so the owner can paste it into Secret
Manager. With --write-env, the script updates a local .env file for local smoke
tests and avoids printing the refresh token.
"""
...
def main() -> None:
    ...
    if args.write_env:
        ...
        if not client_id or not client_secret:
            parser.error(
                "--write-env requires --client-id and --client-secret, or --client-secrets."
            )
        ...
        values = {
            **ENV_DEFAULTS,
            ...
            "GOOGLE_ADS_LOGIN_CUSTOMER_ID": args.login_customer_id
            if args.login_customer_id is not None
            else current.get("GOOGLE_ADS_LOGIN_CUSTOMER_ID", ""),
        }
        _write_env(env_path, values)
        print(f"Updated {env_path} with Google Ads OAuth values.")
